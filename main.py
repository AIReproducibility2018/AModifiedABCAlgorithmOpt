...
def run(pool, r, run_file, REPRO_number_of_runs=None, counter=0):
    ...
    # sort the (e.g 30) results, as they were exucted in parallel.
    # Each of the 30 runs (to be averaged over) are independent, and
    # this has the benefit that the output file will be mostly identical
    # across multiple main.py invocations with the same seed, even if the
    # parallelism causes the execution order to be different.
    # (convert the numpy array to a list, because if funcval is equal when
    #  sorting, then .sort will error out when trying to compare the arrays).
    runs = [(funcval, list(components)) for funcval,components in runs]
    runs.sort()

    # funcvals are the final values of the invocation of bee (after optimizing)
    # all_components is the final values of each parameter in DIM being optimized
    #
    # by default there should be 30 funcvals, and 30 lists of length 10 of components
    funcvals, all_components = zip(*runs)
    all_components = [list(x) for x in all_components]
    with open(run_file, 'a') as f:
        obj = json.dumps([funcvals, all_components, paramdict], sort_keys=True)
        f.write(obj + '\n')
